...
with tab_graphs:
    ...
    if df_earn.empty:
        ...
        # ==========================
        st.subheader("🇺🇸 Tax Prep: Deductible Overtime (FLSA)")
        st.markdown("""
        **Instructions:** Enter this amount if your tax software asks for the *"half amount"* of your time-and-a-half overtime (The 0.5 premium portion).
        ...
        # 1. Select Tax Year
        tax_year = st.selectbox("Select Tax Year", [current_year - 1, current_year], index=0, key="tax_year_select_graphs")

        # 2. Filter by Year (using already fetched df_earn)
        df_tax = df_earn[df_earn['pay_date'].dt.year == tax_year].copy()

        if not df_tax.empty:
            # 3. Calculate Deductible Portion
            # Logic: 
