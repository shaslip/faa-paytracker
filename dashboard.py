...
models.setup_database() # Ensure DB exists

# --- TABS ---
tab_audit, tab_ingest = st.tabs(["🧐 Audit & Time", "📥 Ingestion"])

with tab_audit:
    st.header("Deep Dive Audit")
    stubs = models.get_paystubs_list()
    
    if not stubs.empty:
        # Audit Status Map (Cached logic check)
        status_map = {}
        for sid in stubs['id']:
            d = models.get_full_paystub_data(sid)
            f = logic.run_audit_math(d)
            status_map[sid] = "🔴" if f else "✅"

        def fmt(rid): 
            r = stubs[stubs['id']==rid].iloc[0]
            return f"{status_map[rid]} {r['period_ending']} (Net: ${r['net_pay']:,.2f})"

        sel_id = st.selectbox("Select Pay Period", stubs['id'], format_func=fmt)
        
        # Load Data
        act_data = models.get_full_paystub_data(sel_id)
        act_flags = logic.run_audit_math(act_data)
        ...
        with c1:
            ...
            if exp_data: st.markdown(views.render_paystub_html(exp_data, mode="expected"), unsafe_allow_html=True)
        with c2:
            st.markdown("### Official Paystub")
